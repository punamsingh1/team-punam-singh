...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.BaseSyntheticEvent) => {
    ...
    try {
      ...
      // Professional Error Handling: Removing 'any'
      const errorMessage = error instanceof Error ? error.message : "An unexpected error occurred";
      console.error("Critical Auth Error:", errorMessage);
      alert(errorMessage);
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
      <div className="w-full max-w-md p-8 bg-white shadow-xl rounded-2xl border border-gray-100">
        <header className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1 uppercase tracking-widest">TTTEEEE Security Protocol</p>
        </header>
